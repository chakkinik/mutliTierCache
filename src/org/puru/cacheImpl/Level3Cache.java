...
public class Level3Cache implements Cache {
	private EvictionPolicy policy;
	private static final int CAPACITY = 6;
	private static final int ACCESS_TIME = 3;
	private static final int REFERSH_TIME = 3;

	private Map<String, String> level3Store = new HashMap<>();

	@Override
	public Optional<CacheData> get(String key) {
		if (level3Store.get(key) != null) {
			return Optional.of(new CacheData(level3Store.get(key), ACCESS_TIME));
		}
	...
	public int set(String key, String value) {
		...
		if (level3Store.get(key) != null) {
			level3Store.put(key, value);
		}

		if (level3Store.size() >= CAPACITY) {
			String evictedKey = policy.evict();
			level3Store.remove(evictedKey);
		}
